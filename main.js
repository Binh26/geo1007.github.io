...
function onMapClick(e) {
  ...
  navigator.geolocation.getCurrentPosition(function(position) {
    ...
    var popupContent = ''
      ...
      // +'<button type="button" onclick="insertWFS('+lng+','+lat+', document.getElementById(\'event_name\').value, document.getElementById(\'reported_by\').value )">Insert point</button>' <br>'
      + '<label>Your Coordinates:</label><br>'
      + '<label>Latitude: ' + userLat.toFixed(6) + '</label><br>'
      + '<label>Longitude: ' + userLng.toFixed(6) + '</label><br>'
      + '<button type="button" onclick="' + js_function + '">Insert point</button><br>'
      + '<label>Latitude: ' + lat.toFixed(6) + '</label><br>'
      + '<label>Longitude: ' + lng.toFixed(6) + '</label>';
